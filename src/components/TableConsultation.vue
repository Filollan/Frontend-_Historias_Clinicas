<template>
      ...
      <template #header>
        <div class="header-content">
          <span class="p-input-icon-left">
            ...
            <InputText v-model="globalFilterValue" @input="buscarConsultas" placeholder="Buscar Consultas" />
          </span>
        </div>
      </template>
  ...
  <Dialog v-model:visible="mostrarModalVerConsulta" header="Ver Consulta" :style="{width: '50vw'}" :modal="true">
    ...
    <template #footer>
      <Button label="Verificar Integridad" icon="pi pi-check-circle" @click="verificarIntegridad" class="p-button-text" />
      <Button label="Imprimir PDF" icon="pi pi-file-pdf" @click="imprimirPDF" class="p-button-text" />
      <Button label="Enviar WhatsApp" icon="pi pi-whatsapp" @click="enviarWhatsApp" class="p-button-text" />
...
const buscarConsultas = () => {
  ...
  if (searchTerm === '') {
    filteredConsultations.value = [...consultas.value];
  } else {
    filteredConsultations.value = consultas.value.filter(consulta => {
      return Object.values(consulta).some(value =>
        value && value.toString().toLowerCase().includes(searchTerm)
      );
    });
  }
  console.log('Consultas filtradas:', filteredConsultations.value); // Debugging log
};
...
const abrirModalModificarConsulta = (consulta) => {
  consultaSeleccionada.value = { ...consulta };
  console.log('Consulta seleccionada para modificar:', consultaSeleccionada.value); // Verifica si todos los campos están presentes
  mostrarModalModificarConsulta.value = true;
};

const cerrarModalModificarConsulta = () => {
  mostrarModalModificarConsulta.value = false;
};

const abrirModalEliminarConsulta = (consulta) => {
  consultaSeleccionada.value = { ...consulta };
  console.log('Consulta seleccionada para eliminar:', consultaSeleccionada.value); // Para verificar el ID
  mostrarModalEliminarConsulta.value = true;
};
...
// Verificar integridad de la consulta con blockchain
const verificarIntegridad = () => {
  const hash = consultaSeleccionada.value.diagnosisHash; // Este es el hash generado previamente para la consulta
  console.log('Verificando integridad del hash:', hash);
  
  if (!hash) {
    alert('Error: No se encontró el hash de la consulta.');
    return;
  }

  const esIntegro = api.verifyConsultationIntegrity(hash); // Llama a tu función de blockchain para verificar la integridad
  if (esIntegro) {
    alert('La consulta es íntegra. No ha sido modificada.');
  } else {
    alert('Advertencia: La consulta ha sido modificada.');
  }
};
